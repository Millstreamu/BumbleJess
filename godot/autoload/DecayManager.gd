...
var debug_show_clusters := false  # toggle to visualize cluster ownership
...
func _set_cluster_metadata(c: Vector2i, cluster_id: int) -> void:
        ...
        if not _world.has_node("HexMap"):
                return
        var hexmap: TileMap = _world.$HexMap
        if hexmap == null:
                return
...
func _ensure_cluster_fx_tile(cl_id: int) -> String:
        ...
        if _world != null and _world.has_method("tileset_add_named_color"):
                ...
                if Engine.has_singleton("TileSetBuilder") and TileSetBuilder.has_method("add_color_tile"):
                        TileSetBuilder.add_color_tile(name, col)
                        if _world != null and _world.has_method("rebind_tileset"):
                                _world.rebind_tileset()
