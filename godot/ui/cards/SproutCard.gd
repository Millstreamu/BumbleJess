...
func _refresh_layout() -> void:
	...
	var scale_factor: float = clamp(inner_w / max(base_size.x - 32.0, 1.0), 0.6, 1.8)

	_name_target_font = clamp(roundi(20.0 * scale_factor), 16, 26)
	_text_target_font = clamp(roundi(14.0 * scale_factor), 11, 18)
...
func _set_desc_line_limit(available_height: float) -> void:
	if not is_instance_valid(_desc):
		return
	if available_height <= 0.0:
		_set_rich_text_visible_lines(_desc, 0)
		return
	var line_height: float = _get_rich_text_line_height(_desc)
	if line_height <= 0.0:
		_set_rich_text_visible_lines(_desc, -1)
		return
	var max_lines: int = int(floor(available_height / line_height))
	if max_lines <= 0:
		_set_rich_text_visible_lines(_desc, 1)
	else:
		_set_rich_text_visible_lines(_desc, max_lines)
